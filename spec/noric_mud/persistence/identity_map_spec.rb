  ...
  module Persistence
    describe IdentityMap do
      ...
      context "with a value set that has a strong reference" do
        ...
        it { subject[@key].should eq(@value) } unless defined? JRUBY_VERSION # cannot force garbage collection in jruby
      end
      ...
      context "with a value set that has no strong reference" do
        ...
        it { subject[@key].should be_nil } unless defined? JRUBY_VERSION # cannot force garbage collection in jruby
      end
    end
  end
end
