...
describe Network do
  ...
  it "runs on a given port" do
    expect { Network.new next_port }.to_not raise_error
  end
  ...
  it "won't run two servers on the same port" do
    port = next_port
    expect { Network.new port }.to_not raise_error
    expect { Network.new port }.to raise_error
  end

  context "a server running on a port" do
    before :each do
      @server_port = next_port
      @server = Network.new @server_port
    end
