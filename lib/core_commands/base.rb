...
module CoreCommands
  ...
  add_cmd "north", ->(game, mob, rest, match) { game.exit_room(game, mob,mob.room.north,"north") }
  add_cmd "south", ->(game, mob, rest, match) { game.exit_room(game, mob,mob.room.south,"south") }
  add_cmd "up", ->(game, mob, rest, match) { game.exit_room(game, mob,mob.room.up,"up") }
  add_cmd "down", ->(game, mob, rest, match) { game.exit_room(game, mob,mob.room.down,"down") }
  add_cmd "west", ->(game, mob, rest, match) { game.exit_room(game, mob,mob.room.west,"west") }
  add_cmd "east", ->(game, mob, rest, match) { game.exit_room(game, mob,mob.room.east,"east") }
